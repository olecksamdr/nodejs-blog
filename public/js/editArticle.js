...
function docLoaded () {
	// показуємо кнопки які було прихованні для звичайного користувача
	var showControls = require('jsmodules/utils').showControls;
	showControls();

	var colorizeCode = document.querySelectorAll('.colorize');
    for (var i = 0; i < colorizeCode; i++) {
        colorizeCode[i].addEventListener('click', function (e) {
...
// ВІДПРАВЛЯЄМО ВСЕ НА СЕРВЕР
    ...
    save.onclick = function () {
        require('jsmodules/utils').delControls();
        // набір ф-цій для ідправки даних на сервер
		var ajax = require('jsmodules/ajax/ajax');
        
        var articleID = window.location.href.match(/editArticle\/(.+)/)[1];
        ...
        if (img) {
            if (/^data:image\/jpeg;base64/.test(img.src)) {
                var orig = img.src;
                ...
                bannerName = 'small' + img.getAttribute('data-name');
            }
        }
        ...
        if (images) {
            for (var i = 0; i < images.length; i++) {
                ...
                if (/^data:image\/jpeg;base64/.test(src)) {
       ...
       var code = document.getElementsByTagName('code');
        ...
        ajax.updateHTML({title: title, body: html, desc: description, banner: bannerName, id: articleID}, fullScreen);
